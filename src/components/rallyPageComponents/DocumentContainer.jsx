...
const FolderItem = ({ number, title, files }) => {
    ...
    return (
        <div className="flex flex-col mb-5 min-w-[500px]">
            <li className={`flex w-full p-2 border-b-2 border-gray-300 text-xl items-center font-normal ${isOpen ? 'border-rally-primary' : ''}`}>
                <button
                    className="flex"
            ...
            <ul className={`grid transition-all duration-300 ease-in-out ${isOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
                <div
                    className={`flex-col pl-10 text-lg overflow-hidden`}
    ...
    return (
        <section className="w-full min-h-20 bg-white sm:p-14 p-10 flex justify-center">
            <div className="lg:w-[1024px] w-full overflow-x-auto">
                ...
                <div className="flex flex-col mt-10 w-full text-[#4e4e4e] font-light overflow-x-auto">
                    {loading && <Loader />}
